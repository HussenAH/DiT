...
class DiPT(nn.Module):
    ...
    def forward(self, x, v, xt, yt, xc, yc, t, y):
        """
        Forward pass of DiT.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
        ...
        # print(c==t)
        for block in self.blocks:
            target = block(target, ctx, c)  # (N, T, D)
            # x = block(x, c)  # (N, T, D)
        v = self.final_layer(target, c)  # (N, T, patch_size ** 2 * out_channels)
        # x = self.unpatchify(x)  # (N, out_channels, H, W)
        return v
...
class DiPT_Wrap(nn.Module):
    ...
    #def forward(self, batch, t, y, num_samples=None, reduce_ll=True):
    def forward(self, x, t, y, batch,  num_samples=None, reduce_ll=True):
        """
        num_ctx = None
        batch.x
        batch.y

        batch.xc = batch.x[:, :num_ctx] # pc (position context)
        batch.yc = batch.y[:, :num_ctx] # vc (value context)

        batch.xt = batch.x[:, num_ctx:] # pt (position target)
        batch.yt = batch.y[:, num_ctx:] # vt (value target) - x
        ...
        x = batch.x  # not in use for images
        v = batch.y
        xc = batch.xc
        yc = batch.yc  # not in use for images -> replaced by the value of the indexes
        xt = batch.xt
        yt = batch.yt

        v = self.x_embedder(v)

        xc = pos_embd[:,  batch.xc, :]
        yc = v[:,  batch.xc, :]

        xt = pos_embd[:, batch.xt, :]
        yt = v[:, batch.xt, :]
        ...
        """
        x: (N, T, D), where T = H * W / patch_size ** 2 -> maybe it's also a vt (value target)
        pc: position context,
        vc: value context (N, T, D)
        pt: position target, 
        t: (N,) tensor of diffusion timesteps
        y: (N,) tensor of class labels
        """

        v = self.dit_model(x, v, xt, yt, xc, yc, t, y)  # self.dit_model(x, t, y)
        v = self.unpatchify(v)  # (N, out_channels, H, W)
        return v

    def forward2(self, x, t, y, ctx, ctx_indexes): # forward(self, x.t, t, y, batch.y, cx):
        """
        Forward pass of DiT.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
